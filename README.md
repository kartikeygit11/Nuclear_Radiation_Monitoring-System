...
- **Distance Check** – Warns if a reactor ≤ 500 km from you crosses a risk threshold.  
...
- **Safety Score** – Each row’s `Age` is mapped to a safety tier:
  ...
  - 🔴 **Dangerous**: `≥ 40 years`
- **Distance Check** – `geopy.distance.geodesic` computes the distance (km) between your location and each plant.
- **Map Render** – Plants are visualised as coloured 30 km radius circles in `folium`; your position is a custom icon.
- **Alerts** – Any plant within `≤ 500 km` tagged **Dangerous** triggers an in‑app warning and an OS desktop notification via `plyer`.
...
- [ ] Integrate live radiation APIs (e.g., EPA RadNet)
